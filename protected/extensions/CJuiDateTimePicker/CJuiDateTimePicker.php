...
class CJuiDateTimePicker extends CJuiDatePicker
{
	...
	public function run()
	{
		...
		$js = "jQuery('#{$id}').{$this->mode}picker($options); $('.ui-datepicker-trigger').addClass('btn');";

		if (isset($this->language)){
			$this->registerScriptFile($this->i18nScriptFile);
			$js = "jQuery('#{$id}').{$this->mode}picker(jQuery.extend({showMonthAfterYear:false}, jQuery.datepicker.regional['{$this->language}'], {$options})); $('.ui-datepicker-trigger').addClass('btn');";
		}
